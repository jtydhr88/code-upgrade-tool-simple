...
@Component(property = { "file.extension=properties" }, service = PropertiesFile.class)
public class PropertiesFileJava implements PropertiesFile {
	...
	public List<SearchResult> findProperties(String key) {
		...
		if (infos != null) {
			for (KeyInfo info : infos) {
				retval.add(new SearchResult(_file, info.offset, info.offset + info.length, info.lineNumber,
						info.lineNumber));
			}
		}
		return retval;
